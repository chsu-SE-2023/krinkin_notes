...
Router::Router(std::array<int, cli_cap>& clients) : Gateway(clients) {
    set_defaults();
}
...
Router::Router(std::array<int, cli_cap>& clients, std::array<int, 5>& address) : Gateway(clients, address), WLRepeater() {
    set_defaults();
}

Router::Router(const double*& packets, std::array<int, cli_cap>& clients, std::array<int, 5> address, std::string protocol, std::string ssid, std::string passwd) : Gateway(packets, clients, address, protocol), WLRepeater(ssid, passwd) {
    set_defaults();
}
