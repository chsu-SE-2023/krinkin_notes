...
const char* Tests::memory() { // Done
    return errors[0];
}

const char* Tests::data() { // Done
    ...
    if (r_units[1]->is_wps() != wpa) return errors[11];
    ...
    if (r_units[2]->is_wps() != r_def->is_wps()) return errors[11];
    return errors[0];
}
...
const char* Tests::sort() {
    ...
    Router** lr_units = new Router*[3] {
        new Router(this->packets, clients1, this->address, this->protocol, this->ssid, this->passwd),
        new Router(this->packets, clients2, this->address, this->protocol, this->ssid, this->passwd),
        new Router(this->packets, clients3, this->address, this->protocol, this->ssid, this->passwd)
    };
    ServerRoom<Router> r_cont = ServerRoom<Router>();
    for (int i=0; i<3; i++)
