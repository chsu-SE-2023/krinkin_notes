...
template int ServerRoom<Switch>::get_room_size();
...
template int ServerRoom<Router>::get_room_size();
...
// Инстанцирование для Switch
template<> 
const Switch* ServerRoom<Switch>::search(const int* address) {
    /*
...
// Инстанцирование для Router
template<> 
const Router* ServerRoom<Router>::search(const int* address) {
    /*
     * Функция поиска объекта T по адресу. Возвращает адрес объекта
     * или nullptr, если объект не найден.
     */
...
template <typename T>
int ServerRoom<T>::get_room_size() {
    /*
